...
def get_sentence_alignments(sentence_alignments, xml_folder,
                            outputfile, src, tgt):
    # prepare outputs
    en_out = gzip.open(outputfile+"."+src+".gz", "wb")
    fr_out = gzip.open(outputfile+"."+tgt+".gz", "wb")
    if "/" in outputfile:
    ...
    if len(current_fromto)==2:
        ...
        if imdb_match.group(1) not in imdb:# and int(year) in years:
            ...
            if year in years:
                os.sys.stderr.write("Skipping episode. Episode already included elsewhere.\n")
    ...
    for line in file_pointer:
        ...
        if sentence_match:
            ...
            if sid!=len(sentences)+1:
                os.sys.stderr.write("sentence id = "+str(sid)+"\n")
                os.sys.stderr.write("number of sentences = "+str(len(sentences))+"\n")
                os.sys.stderr.write("There has been an awful error in the sentence id numbers!!!\n")
                input()
...
def write_sentences(xml_folder, alignments, from_file, to_file,
    ...
    for en_sentnums, fr_sentnums in alignments:
        prob_fic = False
        for en_sentnum, fr_sentnum in izip(en_sentnums, fr_sentnums):
